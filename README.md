...
> **模型选型（2026-09-24 实测）**：视觉转写**只用 `gpt-5.6-luna`**。新一代 `gpt-6-luna` 在文本任务上与 5.6 同档（甚至快 15%），但**转写图像时会重述而非照抄**——同 7 页扫描书与历史存档的相似度只有 0.33-0.79（5.6 是 0.96-0.99），会丢行、换人名、自造小标题。加严提示词（逐字照抄／看不清用□）只能把部分页救到 0.61-0.85，仍不可用于存档。→ 转写走 5.6-luna，6-luna 留给纯文本任务（翻译/蒸馏/导读）。
