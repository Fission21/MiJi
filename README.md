...
> **v2.0 产出配方**：6 种产出模式 × 三问菜单（**给谁看 / 什么语言 / 什么形态**）——AI 侧＝skill·知识库；人侧＝**全书翻译**（纯译 / 双语对照可选）· **人类导读**；外加综合套餐。**引擎可选**：agent 子代理 / luna API / 本地模型（实测质量同档，luna 快 4-5 倍）
...
4. **双引擎全本对撞**：484 页扫描书，本地 OCR vs 云端 VLM 全文各跑一遍，字符级一致率 ~97%（Demo 四）
5. **视频 + 书双源融合**：1982 贝尔实验室纪录片 + 《Unix 编程艺术》→ 同一主题「Unix」知识库（Demo 五）
6. **JAR 蒸馏**：gson / h2 / jsoup 等 5 样本全链路 + 跨平台验证 + 蒸馏品鉴（Demo 六）
7. **491 页英文书全套中文化**：知识库 + 全书翻译 + 双语对照 + 精华导读（Demo 七，v2.0 综合套餐首跑）
...
├─────────────────────────────────────────────────────────────┤
│  ④ 产出选择（三问菜单：给谁看 / 什么语言 / 什么形态，v2.0）      │
├─────────────────────────────────────────────────────────────┤
│  ⑤ 按选择产出（可多选、可并行）:                                │
│     🤖 AI 侧: skill（SKILL.md+references）/ 知识库（TOPIC+源）   │
│     👤 人 侧: 全书翻译（纯译/双语对照）/ 人类导读（万字精华）      │
│     🔄 综合套餐: 以上全要（DDIA 491 页首跑）                    │
├─────────────────────────────────────────────────────────────┤
...
> **模型选型（2026-09-24 系统横评：10 图 × 2 模型）**：图像转写**仍用 `gpt-5.6-luna`**。
> - **中文扫描页**：5.6 与历史存档相似度 **0.96-0.99**；`gpt-6-luna` 默认档只有 **0.33-0.60**，且 43%-67% 的输出是原文里没有的字（丢行、换人名、自造小标题、整页主题级编造）。
> - **不是通用视觉退化，是 CJK 读取弱**：同一提示词转写英文书页，6-luna 与 5.6 输出相似度 **0.990**（几乎逐字相同）。
> - **`detail: "high"` 能大幅救回**：6-luna 中文页升到 **0.84-0.91**（p300 0.34→0.84），但 5.6 同页 0.98，仍差一档；`detail: "low"` 时 6-luna 直接拒答（「原图文字过小，无法可靠辨认」）。
> - **图表/截图理解两者接近**，但 6 细节更弱：图中数字会错位（`17055506`→`1705506`）、截图文件名少字符、输出量约为 5.6 的一半。
> - 一句话：**存档级转写 → 5.6-luna；英文页/纯文本任务（翻译/蒸馏/导读）→ 6-luna，还快 ~15%**；非要用 6 看图务必带 `detail: high`。
...
### 📕 案例 Demo 七：491 页英文书 → 全套中文产出（知识库 + 翻译 + 对照 + 导读，2026-09-18）

**《Designing Data-Intensive Applications》**（Kleppmann，491 页英文 PDF）——首次跑通 v2.0 的「综合套餐」：

| 环节 | 做法 | 实测 |
|------|------|------|
| 解析 | 本地 MinerU pipeline | **10 分钟**，549 标题 + 108 插图入库 |
| 知识库 | `kb.py` 三源（英文原文 / 中文全译 / 章节笔记）| TOPIC.md + 双行号锚点，中英均可检索 |
| 全书中译 | **12 章并行子代理**（SPEC 统一规范）| 94 万字节，~25 分钟 |
| 双语对照 | **DP 序列对齐**（Needleman-Wunsch，按数字/术语特征）| 11 章 **3506 段 1:1 对齐**（96.5%+）|
| 精华导读 | 3 子代理分 Part 并行写作 | **1.9 万字** |
| 引擎对比 | agent 子代理 vs luna API | **质量打平**（抽查 15/15）；luna 快 **4-5 倍**（80 秒/章）|

**方法沉淀**：SPEC.md 统一规范（术语表 + 格式铁律 + 分段写入自检）→ 已固化进 SKILL.md Step 2c；对照版对齐算法与引擎实测数据见 SKILL.md「引擎选择」。产出形态：AI 侧知识库 + 人侧三个阅读版本（全译 / 对照 / 导读）。
