...
- **引擎选择（2026-09-18 实测打平）**：同一任务可用三种引擎跑——① **agent 子代理**（默认，Hermes 主模型，≤10 并行，~6 分钟/章）② **luna（gpt-5.6-luna，任意 OpenAI 兼容端点，`/v1/responses`）**：**~80 秒/章**（快 4-5 倍），实测与 agent 版质量打平（ch01：结构 26/26、术语数字抽查 15/15、段落 220 vs 216；导读/笔记同样打平）；③ 本地 Qwen（隐私/离线，笔记 ~3.5 分钟/章）。luna 调用要点：输出取 `output[].content[].output_text`；自带 reasoning（计入输出 token）；max_output_tokens 要给足（一章翻译输出 ~3-4K）；本机通道 = New API :3000（容器重启：`docker start opencode-pool-postgres opencode-pool-redis opencode-pool-new-api`）
- **gpt-6-luna（2026-09-24 实测）**：文本任务（翻译/蒸馏/导读）**可用**——同一段 8K 字符英文正文，6-luna 15.6s / 5.6-luna 18.3s，段落数、数字覆盖（5/5）、术语命中一致，质量同档略快；**视觉转写任务不要用**（见下方「云端视觉识别」保真度警告）。走 opencode Go 套餐时只支持 `/v1/responses`（chat/completions 返回 `Endpoint is unavailable`），且必须带 `x-opencode-session` 头（任意字符串），否则报 `MissingSessionID`
...
| 5 | **人类导读** | 人 | 万字级精华导览（不读全书的人）| ✅ 已跑通（3 子代理分 Part 并行；DDIA 实测 1.9 万字）|
...
- ⚠️ **图像转写仍用 gpt-5.6-luna（2026-09-24 系统横评，10 图 × 2 模型）**：
  - **中文扫描页逐字转写**：5.6 与存档相似度 **0.96-0.99**；**6-luna 默认档只有 0.33-0.60**，且输出的 43%-67% 是原文里没有的字（丢行、把「施瓦岑贝格」写成「格奈森瑙」、自造小标题、甚至整页主题级编造）
  - **不是通用视觉退化，是 CJK 文字读取弱**：同一提示词转写英文书页，6-luna 与 5.6 输出相似度 **0.990**（几乎逐字相同）
  - **`detail: "high"` 能大幅救回**：6-luna 加 `{"type":"input_image","image_url":...,"detail":"high"}` 后中文页升到 **0.84-0.91**（p300 0.34→0.84、p040→0.90、p200→0.91），但 5.6 同页 0.98，仍差一档；`detail:"low"` 时 6-luna 直接拒答（「原图文字过小，无法可靠辨认」，25 字）
  - **图表/截图理解两者接近，但 6 细节更弱**：读图里的数字会错位（用户 id `17055506` → `1705506`）、截图文件名少字符（`cc-cu-test.txt` → `c-cu-test.txt`）、输出量约为 5.6 的一半；页面总结出现过主题级编造（把「军事情报/间谍」那页总结成「经济与全球化风险」）
  - 一句话：**存档级转写 → 5.6-luna；英文页/纯文本任务 → 6-luna；非要用 6 看图务必带 `detail: high`**
...
| 想用 opencode Go 套餐里的 gpt-6-luna 做视觉转写 | **中文页别用，英文页可以**：中文扫描页保真度默认档 0.33-0.60（5.6 是 0.96-0.99），加 `detail: high` 可回到 0.84-0.91 仍不及 5.6；英文页两模型 0.990 等价。Go 套餐端点 `https://opencode.ai/zen/go/v1` 只支持 `/v1/responses` 且必须带 `x-opencode-session` 头；本机 relay 已接通 `opencode-go/gpt-6-luna`（:3000）。纯文本任务 6-luna 可用且更快 |
