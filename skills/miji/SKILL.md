...
- ⚠️ **视觉转写只能用 gpt-5.6-luna，别换 gpt-6-luna（2026-09-24 实测）**：同一页扫描书图，gpt-5.6-luna 与 09-01 存档相似度 0.96-0.99（稳定复现），**gpt-6-luna 只有 0.33-0.79**——它不是「转写」而是「重述」：丢行、把「施瓦岑贝格」写成「格奈森瑙」、自造小节标题（把「政治上取守势而军事上取攻势的战争」写成「政治上取得战争胜利的限制条件」）、大量用「……」填充。加严提示词（逐字照抄/不许改写/看不清用□）只能把部分页从 0.33 拉到 0.61-0.85，仍不可用于存档。→ 转写走 5.6-luna，6-luna 只用在纯文本任务（翻译/蒸馏/导读）
...
| 想用 opencode Go 套餐里的 gpt-6-luna 做视觉转写 | **别用**：保真度只有 5.6-luna 的一半（0.33-0.79 vs 0.97），会改写/编造内容。Go 套餐端点 `https://opencode.ai/zen/go/v1` 只支持 `/v1/responses` 且必须带 `x-opencode-session` 头；本机 relay 已把 `opencode-go/gpt-6-luna` 接通（:3000，用法与 5.6 相同），纯文本任务可用 |
